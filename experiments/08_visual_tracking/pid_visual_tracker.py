...
DISTURBANCE_START_SECONDS = 2.0
DISTURBANCE_TORQUE = 0.1
...
for step in range(number_of_steps):
    if data.time >= DISTURBANCE_START_SECONDS:
        data.qfrc_applied[0] = (
            DISTURBANCE_TORQUE
        )
    else:
    ...
    if step % camera_update_steps == 0:
        ...
        if detected:
            ...
            if (
                ...
                integral_error_u = np.clip(
                    integral_error_u,
                    -INTEGRAL_LIMIT,
                    INTEGRAL_LIMIT,
                )

                integral_error_v = np.clip(
                    integral_error_v,
                    -INTEGRAL_LIMIT,
                    INTEGRAL_LIMIT,
                )
        ...
        if data.time >= next_print_time:
            print(
                ...
                f"{data.ctrl[1]:6.3f}) | "
                f"disturbance={data.qfrc_applied[0]:5.2f} | "
                f"I=({pan_i_term:6.3f}, "
                f"{tilt_i_term:6.3f}) | "
                f"detected={detected}"
            )
